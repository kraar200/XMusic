...
async def useradd(client, message: Message, _):
    if MONGO_DB_URI is None:
        return await message.reply_text(
            f"**بسبب {MUSIC_BOT_NAME} مشكلات الخصوصية ، لا يمكنك إدارة مستخدمي sudo عليها {MUSIC_BOT_NAME} قاعدة البيانات .\n\n الرجاء إضافة قاعدة بيانات mongo الخاصة بك في vars لاستخدام هذه الميزة .**"
        )
...
async def userdel(client, message: Message, _):
    if MONGO_DB_URI is None:
        return await message.reply_text(
            f"**بسبب {MUSIC_BOT_NAME} مشكلات الخصوصية ، لا يمكنك إدارة مستخدمي sudo عليها {MUSIC_BOT_NAME} قاعدة البيانات .\n\n الرجاء إضافة قاعدة بيانات mongo الخاصة بك في المتغير لاستخدام هذه الميزة .**"
        )
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text(_["auth_1"])
    ...
    if removed:
        ...
        await message.reply_text(_["sudo_4"])
        return
    await message.reply_text(f"هناك خطأ ما .")
