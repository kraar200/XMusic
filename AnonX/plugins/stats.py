...
async def gstats_global(client, message: Message, _):
    ...
    title = title.title()
    final = f"الأكثر تشغيل على الأغنية {MUSIC_BOT_NAME}\n\n**عنوان :** {title}\n\nشغل ** {co} **الوقت ."
    ...
    def get_stats():
        ...
        if what in ["Tracks", "Here"]:
            for items, count in list_arranged.items():
                ...
                if items == "telegram":
                    msg += f"🍒 [ᴛᴇʟᴇɢʀᴀᴍ ᴍᴇᴅɪᴀ](https://t.me/DevKiMO) ** شغل {count} الوقت**\n\n"
                else:
                    msg += f"📌 [{title}](https://www.youtube.com/watch?v={items}) ** شغل {count} الوقت**\n\n"
    ...
    if what in ["Users", "Chats"]:
        for items, count in list_arranged.items():
            ...
            try:
                ...
                continue
            limit += 1
            msg += f"💖 `{extract}` شغل {count} مرات على البوت .\n\n"
        temp = (
...
**الوحدات :** {mod}
**الدردشات :** {served_chats} 
**المستخدمين :** {served_users} 
**منعت :** {blocked} 
**sudoers :** {sudoers} 
    
**استفسارات :** {total_queries} 
**مساعدين :** {assistant}
**ᴀss ᴀᴜᴛᴏ ʟᴇᴀᴠᴇ:** {ass}
**وضع النظيف :** {cm} دقائق 

**حد المدة :** {play_duration} دقائق
**حد التحميل :** {song} دقائق
**حد قائمة التشغيل :** {playlist_limit}
...
async def overall_stats(client, CallbackQuery, _):
    if CallbackQuery.from_user.id not in SUDOERS:
        return await CallbackQuery.answer(
            "فقط لمستخدمي sudo .", show_alert=True
        )
    callback_data = CallbackQuery.data.strip()
    what = callback_data.split(None, 1)[1]
    if what != "s":
