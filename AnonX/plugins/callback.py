...
async def unban_assistant_(_, CallbackQuery):
    ...
    if not a.can_restrict_members:
        ...
        try:
            ...
            return await CallbackQuery.answer(
                "فشل في إلغاء حظر حساب المساعد .",
                show_alert=True,
            )
        return await CallbackQuery.edit_message_text(
            "تم إلغاء حظر حساب المساعد بنجاح .\n\nحاول التشغيل الآن..."
        )
    ...
    if command == "Pause":
        ...
        if int(command) in [1, 3]:
            if (duration_played - duration_to_skip) <= 10:
                ...
                return await CallbackQuery.answer(
                    f"» البوت غير قادر على البحث بسبب تجاوز المدة .\n\nشغلت حاليا  :** {bet}** دقائق من **{duration}** الدقائق .",
                    show_alert=True,
                )
        ...
        await mystic.edit_text(
            f"{string}\n\nالتغييرات التي تم إجراؤها بواسطة : {mention} !"
        )
